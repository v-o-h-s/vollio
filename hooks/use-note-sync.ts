...
export interface UseNoteSyncReturn {
   ...
   */
  broadcastUpdate: (noteId: string, updates: { title?: string; content?: JSONContent }) => void;
...
export function useNoteSync(options: UseNoteSyncOptions = {}): UseNoteSyncReturn {
  const {
  ...
  const handleNoteCreate = useCallback((note: Note) => {
    ...
    dispatch(
      apiSlice.util.upsertQueryData("getNote", note.id, note)
    );
  }, [dispatch, enableAutoUpdate]);
  ...
  const handleNoteDelete = useCallback((noteId: string) => {
    ...
    dispatch(
      apiSlice.util.invalidateTags([{ type: "Note", id: noteId }])
    );
  }, [dispatch, enableAutoUpdate]);
  ...
  const broadcastCreate = useCallback((note: Note) => {
    broadcastNoteCreate(note);
  }, []);

  const broadcastDelete = useCallback((noteId: string) => {
    broadcastNoteDelete(noteId);
  }, []);

  const focusNoteInTab = useCallback((noteId: string) => {
  ...
  const navigateToNote = useCallback((noteId: string) => {
    return navigateToNoteInTab(noteId);
  }, []);
