...
export function useQuizErrorHandling(options: QuizErrorHandlingOptions = {}) {
      ...
      switch (appError.type) {
        ...
        case ErrorType.TIMEOUT_ERROR:
          toast.error(
            ...
            retryable ? {
              label: 'Try Again',
              onClick: () => setErrorState(prev => ({ ...prev, error: null }))
            } : undefined
          );
          break;
  ...
  }, [retryable, maxRetries, errorState.retryCount, showToast, handleError, toast]);
  ...
  const handleQuizGenerationError = useCallback((error: any) => {
  ...
  const handleDocumentProcessingError = useCallback((error: any, documentName?: string) => {
    return handleError(error, {
  ...
  const handleQuizSubmissionError = useCallback((error: any, quizId?: string) => {
    return handleError(error, {
  ...
  const handleQuizLoadingError = useCallback((error: any, quizId?: string) => {
    return handleError(error, {
      component: 'QuizPlayer',
      action: 'load_quiz',
      quizId
  ...
  return {
    ...
    isAuthError: [ErrorType.AUTHENTICATION_ERROR, ErrorType.AUTHORIZATION_ERROR].includes(errorState.error?.type as ErrorType),
