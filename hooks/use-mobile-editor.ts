...
export function useMobileEditor(options: MobileEditorOptions = {}) {
  ...
  const { keyboardState, scrollIntoView, adjustElementPosition } = useMobileKeyboard({
    onShow: (height) => {
      setState(prev => ({
        ...prev,
      ...
      // Update CSS variables for keyboard height
      document.documentElement.style.setProperty('--mobile-keyboard-height', `${height}px`);
      document.documentElement.style.setProperty('--mobile-viewport-height', `${keyboardState.viewportHeight}px`);
  ...
  const handleModeChange = useCallback((newMode: EditorMode) => {
    ...
    // Apply mode-specific classes to body
    document.body.classList.remove('editor-normal', 'editor-fullscreen', 'editor-focus');
    ...
    // Handle focus mode specific adjustments
    if (newMode === 'focus') {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'VolumeUp' || event.key === 'VolumeDown') {
        ...
        const nextIndex = event.key === 'VolumeUp' 
  ...
  useEffect(() => {
    return () => {
      // Remove body classes
      document.body.classList.remove('editor-normal', 'editor-fullscreen', 'editor-focus', 'mobile-keyboard-visible');
  ...
  const handleInputBlur = useCallback((element: HTMLElement) => {
    if (!isMobile) return;
  ...
  const handlePullRefresh = useCallback((onRefresh?: () => void) => {
    ...
    const handleTouchEnd = () => {
      if (isPulling && pullRefreshState.pullDistance > pullRefreshState.threshold) {
        onRefresh?.();
        if (enableHapticFeedback) {
          successFeedback();
        }
  ...
  }, [isMobile, enableGestures, enableHapticFeedback, pullRefreshState.pullDistance, pullRefreshState.threshold, successFeedback]);
