...
import { DocumentProcessingStatus as ProcessingStatus } from "@/lib/types";
import { 
  RefreshCw, 
  CheckCircle, 
  AlertCircle, 
...
export function DocumentProcessingStatus({ 
  ...
  const { handleError, executeWithErrorHandling } = useDocumentProcessingErrorHandling();
  ...
  useEffect(() => {
    ...
    const hasProcessingDocs = documentIds.some(docId => {
      ...
      return status && (status.status === "processing" || status.status === "pending");
    });
  ...
  const handleRefresh = async () => {
    ...
    try {
      await executeWithErrorHandling(async () => {
        await onRefresh();
        toast.success("Status Updated", "Processing status refreshed successfully.");
      }, {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span>{completedCount}/{documentIds.length} completed</span>
          </div>
          <Progress 
        ...
        {documentIds.length > 1 && completedCount === documentIds.length && (
          <div className="p-3 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
            ...
            <p className="text-sm text-green-600 dark:text-green-400 mt-1">
              All {documentIds.length} documents are processed and ready for cross-document quiz generation.
            </p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {documentIds.map((docId) => {
            ...
            const progress = status ? 
              (status.totalChunks > 0 ? (status.processedChunks / status.totalChunks) * 100 : 0) : 0;
              ...
              <div key={docId} className="p-3 border rounded-lg">
                ...
                {status && status.status === "completed" && (
                  <div className="text-xs text-muted-foreground space-y-1">
                    ...
                    <p>Method: {formatProcessingMethod(status.extractionMethod)}</p>
                    {status.processingCompletedAt && (
                      <p>Completed: {new Date(status.processingCompletedAt).toLocaleTimeString()}</p>
                    )}
                  </div>
                )}
                
                {status && status.status === "failed" && status.errorMessage && (
