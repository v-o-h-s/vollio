...
import { DocumentFilters } from "../components/FilterDropdown";
...
export function useDocumentsViewState(documents: Document[], folders: Folder[]) {
  ...
  // Filter documents based on search, filters, and current folder
  const filteredDocuments = useMemo(() => {
    return documents.filter((document) => {
      ...
      // Folder filter
      if (currentFolder ? document.folderId !== currentFolder : document.folderId !== null) {
        return false;
      }
      ...
      if (isDocument && !filters.showDocuments) return false;
      if (isDoc && !filters.showDocs) return false;
      if (isImage && !filters.showImages) return false;

      // Source filters
      if (document.isGoogleDriveDocument && !filters.showGoogleDrive) return false;
      if (!document.isGoogleDriveDocument && !filters.showLocal) return false;

      return true;
    });
  }, [documents, searchQuery, filters, currentFolder]);
  ...
  const selectAll = useCallback(() => {
    const allItems: SelectedItem[] = [
      ...
      ...filteredDocuments.map((document) => ({ type: "document" as const, id: document.id })),
    ];
