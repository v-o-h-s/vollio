...
export function useFile() {
  // Query
  const { data: filesData, isLoading, error, refetch } = useGetAllFilesQuery();
  if (error) {
    console.error("Error fetching files:", error);
  }

  // Mutations
  const [renameFileMutation, { isLoading: isRenaming }] =
    useRenameFileMutation();
  const [moveFileMutation, { isLoading: isMoving }] = useMoveFileMutation();
  const [deleteFileMutation, { isLoading: isDeleting }] =
    useDeleteFileMutation();
