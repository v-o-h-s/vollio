/**
 * @file ClassroomImportDialog.tsx
 * @description Dialog component for importing files from Google Classroom.
...
import { Loader2, School, RefreshCcw, ArrowRightCircle, FileText, FolderOpen } from "lucide-react";
import {
...
import { CourseListResponse, CourseContent } from "@/lib/types/server-respones/classroomRouteResponses";
...
export function ClassroomImportDialog({ open, onOpenChange, onImported }: ClassroomImportDialogProps) {
    const { data: classroomStatus, isLoading: isChecking } = useGetGoogleClassroomConnectionStatusQuery();
    ...
    const courseContentQuery = useGetGoogleClassroomCourseContentQuery(selectedCourseId || "", {
    ...
    const [addFileFromDrive] = useAddFileFromGoogleDriveMutation();
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            for (const fileGoogleDriveId of ids) {
                await addFileFromDrive({ fileGoogleDriveId }).unwrap();
            }
                ...
                {!isConnected ? (
                    <div className="space-y-4">
                        <Alert>
                            ...
                            <AlertDescription>
                                Authenticate with your Google account to browse courses and import materials.
                            </AlertDescription>
                        </Alert>
                        <Button onClick={handleConnect} disabled={isChecking} className="gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="md:col-span-1 space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => refetchCourses()} className="gap-1">
                                        ...
                                        <div className="divide-y">
                                            {coursesData.data.map((course: CourseListResponse) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-3 py-2 hover:bg-muted/50 transition ${selectedCourseId === course.id ? "bg-primary/10" : ""
                                                        }`}
                                                >
                                                    <div className="flex items-center justify-between gap-2">
                                                        <span className="text-sm font-medium truncate">{course.name}</span>
                                                        <Badge variant={course.courseState === "ACTIVE" ? "success" : "secondary"}>
                                                            {course.courseState}
                                                        </Badge>
                                                    </div>
                                                    <p className="text-xs text-muted-foreground mt-1">Updated {new Date(course.updateTime).toLocaleDateString()}</p>
                                                </button>
                                            ))}
                                        </div>
                                    </ScrollArea>
                        ...
                        <div className="md:col-span-2 space-y-3">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-sm font-medium text-muted-foreground">
                                        {selectedCourseId ? "Materials" : "Select a course to view materials"}
                                    </p>
                                </div>
                                {selectedCourseId && (
                                    <Button variant="ghost" size="sm" onClick={() => refetchContent()} className="gap-1">
                                        <RefreshCcw className="h-4 w-4" />
                                        Refresh
                                    </Button>
                                )}
                            ...
                            <div className="border rounded-lg h-[420px] overflow-hidden relative">
                                {!selectedCourseId ? (
                                            ...
                                            {materials.map((m) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-sm font-medium truncate">{m.title || "Untitled file"}</p>
                                                        <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                                                            <FileText className="h-3 w-3" />
                                                            {m.source === "announcement" ? "Announcement" : "Course work"}
                                                        </p>
                                                    </div>
                                                </label>
                                            ))}
                                    ...
                                    <div className="h-full flex items-center justify-center text-sm text-muted-foreground px-3 text-center">
                                        No materials with Drive files in this course.
                                    </div>
                                )}
                            ...
                            <div className="flex justify-end gap-2">
                                ...
                                <Button
                                    ...
                                    className="gap-2"
                                >
                                    {isImporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRightCircle className="h-4 w-4" />}
