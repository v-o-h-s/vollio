...
export function ZoomControls({ documentViewerRef }: ZoomControlsProps) {
  ...
  // Update zoom level from Document viewer
  useEffect(() => {
    if (documentViewerRef?.current) {
      ...
      const updateZoomLevel = () => {
        try {
          ...
          console.warn("Could not get zoom level from Document viewer:", error);
        }
      };
  ...
  const handleZoomIn = () => {
    ...
    if (!magnification) {
      console.warn(
        "Magnification module is not available on the Document viewer instance."
      );
      return;
    }
