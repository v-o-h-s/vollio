...
- **UI State**: Visibility of Assistant and Noter panels.
...
- **`useAssistantLogic`**: Manages the AI Assistant chat state, message history, and interactions with the AI API.
- **`useNoterLogic`**: Handles note creation, tab management within the document view, and appending content to notes.
- **`useHighlightActions`**: Provides CRUD operations for PDF highlights via RTK Query.
- **`useViewerUI`**: Simple state for toggling side panels (Assistant/Noter).
...
### Side Panels
- **Assistant Chat**: Located in `components/Assistant/`, provides a chat interface to "talk" to the document.
- **Noter**: Located in `components/notes/`, provides a tabbed note-taking interface integrated directly with document content.

## Features & Workflows

### 1. AI Insights
Users can select text and choose "Add to V-Notes". This:
1. Generates an AI explanation (if triggered via Assistant).
...
### 3. Contextual Assistant
The Assistant can "Explain" selected text. The query is automatically populated with the selected text and document metadata, providing grounded AI responses.
...
- **`Message`**: Defines the structure for Assistant chat messages, including metadata for document references.
