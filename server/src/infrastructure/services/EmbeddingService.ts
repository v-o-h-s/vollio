...
export class EmbeddingService implements IEmbeddingService {
    ...
    async generateEmbeddingForText(text: string): Promise<number[]> {
        ...
        if (!res || !Array.isArray(res.data) || res.data.length === 0) {
            throw new Error(`Failed to generate embedding: unexpected response shape: ${JSON.stringify(res)}`);
        }

        const item = res.data[0] as EmbeddingItem;
        if (!Array.isArray(item.embedding)) throw new Error("Failed to generate embedding: missing embedding in response item");
