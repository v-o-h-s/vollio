...
export class CreateQuizUseCase {
  ...
  async execute(data: CreateQuizDTO): Promise<CreateQuizResponse> {
    ...
    refinedPrompt = await this.generativeAiService.refineUserPrompt(
      data.userPrompt
    );
    const chunks = await this.semanticSearchService.findRelevantChunks(
      refinedPrompt
    );
    ...
    this.logger.info(
      `Generated ${quizQuestions.length} questions for the quiz.`
    );
