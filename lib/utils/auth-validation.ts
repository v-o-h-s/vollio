  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Could not decode JWT token for expiry check:",
          decodeError
        );
      }
    } catch (tokenError) {
  ...
  try {
    ...
    try {
      ...
      if (error) {
        ...
        const allOwnFiles =
          data?.every((pdf) => pdf.user_id === userId) ?? true;
        results.push({
          test: "PDF RLS - Own files only",
          passed: allOwnFiles,
          error: allOwnFiles
            ? undefined
            : "Found PDFs belonging to other users",
        });
      }
    } catch (error) {
      results.push({
    ...
    try {
      ...
      if (error) {
        ...
        const allOwnActivity =
          data?.every((activity) => activity.user_id === userId) ?? true;
        results.push({
          test: "Activity RLS - Own activity only",
          passed: allOwnActivity,
          error: allOwnActivity
            ? undefined
            : "Found activity belonging to other users",
        });
      }
    } catch (error) {
      results.push({
    ...
    try {
      const fakeUserId =
        "fake_user_id_" + Math.random().toString(36).substring(7);
      const { error } = await supabaseClient.from("pdfs").insert({
        user_id: fakeUserId,
        filename: "test.pdf",
        file_size: 1000,
      ...
      if (token) {
        ...
        results.push({
          ...
          error:
            tokenUserId === userId
              ? undefined
              : `Token user ID ${tokenUserId} doesn't match ${userId}`,
        });
      } else {
        results.push({
          test: "JWT Token - Correct user ID",
