...
async function demonstrateEmbeddingService() {
  ...
  try {
    ...
    if (batchResult.success) {
      ...
      console.log(
        `   - Average quality score: ${
          batchResult.averageQualityScore?.toFixed(3) || "N/A"
        }`
      );
      console.log(
        `   - Cache hit rate: ${(batchResult.cacheHitRate * 100).toFixed(1)}%\n`
      );
    } else {
    ...
    const cachedResult = await embeddingService.generateEmbedding(
      cacheTestText,
      {
        cacheEnabled: true,
      }
    ...
    console.log(
      `   - Average age: ${cacheStats.averageAge.toFixed(2)} hours\n`
    );
    ...
    activeModels.forEach((model) => {
      console.log(
        `   - ${model.name} v${model.version} (${model.dimensions}D, max ${model.maxTokens} tokens)`
      );
    });
